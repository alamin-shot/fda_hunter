...
import { dashboardApi, ProfileData } from '@/services/dashboardApi'
  ...
  return (
    <div className="bg-[#0E121B] p-6 rounded-[24px]">
      ...
      <div className="flex items-center gap-4 mt-4.5">
        ...
        <button onClick={() => fileInputRef.current?.click()} className="text-white text-sm font-medium py-1.5 px-4 border border-[#2B303B] rounded-lg cursor-pointer hover:bg-white/10 transition-colors">
          {profileImage ? 'Change' : 'Upload'}
        </button>
        {profileImage && (
          <button onClick={() => { setProfileImage(null); setImagePreview(null) }} className="text-red-400 text-sm font-medium py-1.5 px-4 border border-red-500/50 rounded-lg cursor-pointer hover:bg-red-500/10 transition-colors">
            Remove
          </button>
        )}
      </div>
      ...
      <div className="mt-6 p-4 border border-[#323B49] rounded-lg">
        ...
        <div className="flex items-center gap-5 mt-4">
          <div className="flex flex-col gap-2 flex-1 relative">
            ...
            <input type="text" id="first-name" value={formData.first_name}
              onChange={(e) => setFormData(prev => ({ ...prev, first_name: e.target.value }))}
              className="py-2.5 px-3 border border-[#2B303B] rounded-lg text-white bg-transparent" placeholder="first name" />
          </div>
          <div className="flex flex-col gap-2 flex-1 relative">
            <label htmlFor="last-name" className="text-sm text-white">Last Name</label>
            <input type="text" id="last-name" value={formData.last_name}
              onChange={(e) => setFormData(prev => ({ ...prev, last_name: e.target.value }))}
              className="py-2.5 px-3 border border-[#2B303B] rounded-lg text-white bg-transparent" placeholder="last name" />
          </div>
        </div>
