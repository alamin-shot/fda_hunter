...
export default function Password() {
    ...
    const handleUpdate = async () => {
        ...
        setLoading(true)
        try {
            const response = await dashboardApi.changePassword(form)
            if (response.status) {
        ...
        <div className="bg-[#0E121B] p-6 rounded-[24px]">
            ...
            <div className="my-8 space-y-6">
                {['current_password', 'new_password', 'new_password_confirmation'].map((field, i) => (
                    <div key={field}>
                        <label htmlFor={field} className="text-white text-base font-semibold">
                            {i === 0 ? 'Old Password' : i === 1 ? 'New Password' : 'Confirm Password'} <span className="text-[#E03137]">*</span>
                        </label>
                        <input type="password" id={field} value={form[field as keyof typeof form]}
            ...
            <div className="flex justify-end gap-2">
                ...
                <button onClick={() => setForm({ current_password: '', new_password: '', new_password_confirmation: '' })}
