...
export const RecentPredictionColumn = (
...
) => [
  {
    ...
    formatter: (value: any) => {
      ...
      return (
        <span
          className={`px-3 py-1 rounded-lg text-sm font-medium ${colors[name] || "bg-gray-700 text-gray-300"}`}
        >
          {name}
        </span>
      );
    ...
    formatter: (value: string | null, row: any) => {
      ...
      return (
        <select
          ...
          disabled={isResolved}
          className={`px-2 py-1.5 rounded-lg text-sm font-medium outline-none border-none ${
            statusConfig[currentStatus] || "bg-gray-800 text-gray-400"
